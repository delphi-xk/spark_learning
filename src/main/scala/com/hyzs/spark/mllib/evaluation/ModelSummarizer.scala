...
/**
  * :: DeveloperApi ::
  * ModelSummarizer implements [[ModelStatisticSummary]] to compute the mean,
...
class ModelSummarizer extends ModelStatisticSummary with Serializable {


  /**
    * Reference: <a href="http://en.wikipedia.org/wiki/Algorithms_for_calculating_variance">
    * delta = value - E(x_{n-1})
    * E(x_n) = E(x_{n-1}) + delta / N
    * delta2 = value - E(x_n)
    * M(2,n) = M(2,n-1) + delta * delta2
    *
    * currM2N: sum_{i=1}^n {(x - E(x))^2}
    * currM2: sum_{i=1}^n {x^2}
    */
  private var n = 0
...
/*    val localCurrMean = currMean
    ...
    instance.foreachActive { (index, value) =>
      if (value != 0.0) {
        ...
        currMean(index) = prevMean + weight * diff / (weightSum(index) + weight)
  ...
  def merge(other: ModelSummarizer): this.type = {
